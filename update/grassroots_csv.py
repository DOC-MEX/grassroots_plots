...
def getRowCsv(row_json):
    ...
    #print(" ROW phenotypeNames: ", phenotypeNames)
    #print(" ROW RAW values: ", raw_value)
...
def create_CSV(plot_data, phenotypes, treatment_factors, plot_id):
    ...
    #filename = os.path.join(path, name)          MAIN DJANGO
    file_name = f"data-{plot_id}.csv"
    ...
    os.remove(file_path)
    os.rename('data_cleaned.csv', file_path)
